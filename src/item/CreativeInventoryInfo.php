...
final class CreativeInventoryInfo {
	...
	public static function DEFAULT(): self {
		return new self(self::CATEGORY_ITEMS, self::NONE);
	}
    ...
    public static function getStringCategory(int $id): ?string {
        return match ($id) {
            1 => CreativeInventoryInfo::CATEGORY_CONSTRUCTION,
            2 => CreativeInventoryInfo::CATEGORY_NATURE,
            3 => CreativeInventoryInfo::CATEGORY_EQUIPMENT,
            4 => CreativeInventoryInfo::CATEGORY_ITEMS,
            default => null
        };
    }

    /**
     * Returns a PM creative group if it is in the specified category, otherwise null.
     */
     ...
     * Returns a PM creative group if it is in the specified category, otherwise null.
     */
    public static function getPMGroup(string $category, string $oldGroup): ?CreativeGroup { // TODO
        try {
            $groups = CraftingManagerFromDataHelper::loadJsonArrayOfObjectsFile(
                Path::join(BedrockDataFiles::CREATIVE, $category . ".json"),
                CreativeGroupData::class
            );;
        } catch (\RuntimeException) { // file was not found (category does not exist)
            return null;
        }

        foreach ($groups as $groupData) {
            if (($groupName = $groupData->group_name) !== $oldGroup) continue;
            if (is_null($groupData->group_icon)) return null;
            else $icon = CraftingManagerFromDataHelper::deserializeItemStack($groupData->group_icon);

            return new CreativeGroup($groupName, $icon);
        }
